...
%DIF ADD C:\users\simon\onedrive - bth\research\weed\articel\access\WeedArticleI_IEEE_Access__Copy_\access_revised_v2.tex   Fri Apr 24 14:07:54 2026
%DIF < 
%DIF < 
...
An efficient way to collect data for detecting invasive plants such as black-grass is using \DIFaddbegin \DIFadd{unmanned aerial vehicles (UAVs), here referred to as }\DIFaddend agricultural drones. These drones have gradually become an essential tool in precision agriculture and smart farming. They can be utilized for different applications, including crop monitoring, crop spraying, weed detection, and so on \cite{UAV_Agr}. Particularly, agricultural drones are equipped with optical sensors such as RGB, near-infrared (NIR), and other multispectral cameras to capture the necessary images from crops \cite{von2015deploying}. In addition, various vegetation indices \DIFdelbegin \DIFdel{, such as the normalized difference vegetation index (NDVI), }\DIFdelend can be derived from these images \cite{pettorelli2013, hassan2019rapid}.
...
\DIFadd{Classical image-analysis approaches for weed detection often rely on color indices, vegetation indices, spectral thresholds, and handcrafted features rather than learned representations. These approaches remain important because color and vegetation-index methods can emphasize plant material under varying soil, residue, and illumination conditions \mbox{%DIFAUXCMD
...
The different VIs have different properties and are used for different purposes. In our application, they will serve as an additional input source to the neural network to improve the detection of the black-grass. In this work, we focus on the \DIFdelbegin \DIFdel{following VIs : Triangular Greenness Index (TGI ) \mbox{%DIFAUXCMD
\cite{hunt2011remote}}\hskip0pt%DIFAUXCMD
, Normalized Difference Vegetation Index (NDVI ) \mbox{%DIFAUXCMD
\cite{pettorelli2013}}\hskip0pt%DIFAUXCMD
, Excess Green Index (ExG ) \mbox{%DIFAUXCMD
\cite{woebbecke1995color}}\hskip0pt%DIFAUXCMD
and Normalized Difference Water Index (NDWI ) \mbox{%DIFAUXCMD
\cite{gao1996ndwi}}\hskip0pt%DIFAUXCMD
as presented in Table~\ref{tab:vi}}\DIFdelend \DIFaddbegin \DIFadd{four VIs listed in Table~\ref{tab:vi}: TGI \mbox{%DIFAUXCMD
\cite{hunt2011remote}}\hskip0pt%DIFAUXCMD
, NDVI \mbox{%DIFAUXCMD
\cite{pettorelli2013}}\hskip0pt%DIFAUXCMD
, ExG \mbox{%DIFAUXCMD
\cite{woebbecke1995color}}\hskip0pt%DIFAUXCMD
, and NDWI \mbox{%DIFAUXCMD
\cite{gao1996ndwi}}\hskip0pt%DIFAUXCMD
}\DIFaddend . These vegetation indices are calculated as follows:
...
Here, the wavelength in $nm$ is denoted by $\lambda$, the image \DIFaddbegin \DIFadd{intensity }\DIFaddend by $I$, and the \DIFdelbegin \DIFdel{channel with }\DIFdelend \DIFaddbegin \DIFadd{spectral band by }\DIFaddend the subscript. \DIFaddbegin \DIFadd{Thus, $I_{\text{B}}$, $I_{\text{G}}$, $I_{\text{R}}$, and $I_{\text{NIR}}$ refer to pixel values in the aligned blue, green, red, and near-infrared band images, respectively. Each formula was applied pixel-wise to generate a VI image that could be stacked with the original multispectral bands as an additional detector input.
}\DIFaddend 
...
}\DIFaddend . We therefore evaluate whether adding a single \DIFdelbegin \DIFdel{vegetation-index (VI ) }\DIFdelend \DIFaddbegin \DIFadd{VI }\DIFaddend channel to the five-band baseline improves black-grass detection.
...
\DIFaddFL{Multispectral training adaptation }& \DIFaddFL{The training pipeline used stacked multispectral arrays instead of RGB images. RGB-specific HSV augmentation terms were disabled for the multispectral runs. }\\
	\DIFaddFL{Evaluation thresholds }& \DIFaddFL{The precision--recall (PR) and F1-confidence curves sweep the detector confidence score rather than reporting one fixed operating threshold. Average precision (AP) is computed from the resulting PR behavior after matching predicted and annotated instances. }\\
	\DIFaddFL{Bootstrap confidence intervals }& \DIFaddFL{Table~\ref{tab:performance_metrics_by_position} reports 95\% confidence intervals (CIs) from a detection-level nonparametric bootstrap. Matched detection outcomes and detector confidence scores were resampled with replacement 10}{\DIFaddFL{,}}\DIFaddFL{000 times; AP was recomputed for each resample and CIs are percentile intervals. }\\
\bottomrule
\end{tabular}
\end{table}
...
\DIFaddend The data was collected using a multispectral drone, equipped with five single-band cameras with 2-megapixel (2~MP) sensors \DIFdelbegin \DIFdel{: Blue (B), Green (G), Red (R), Red Edge (RE), and Near-Infrared (NIR )}\DIFdelend \DIFaddbegin \DIFadd{corresponding to the B, G, R, RE, and NIR bands defined in Section~\ref{seq:VI}}\DIFaddend . The drone was flown at a speed of 1~m/s and altitudes of 1-2~meters, capturing images with a resolution of 0.6-1.2~mm/pixel and consequently an area of 0.8~m $\times$ 1.0~m to 1.6~m $\times$ 2.0~m. 
...
The primary indices considered were \DIFdelbegin \DIFdel{the Triangular Greenness Index (TGI), Excess Green Index (ExG), Normalized Difference Vegetation Index (NDVI), and Normalized Difference Water Index (NDWI)}\DIFdelend \DIFaddbegin \DIFadd{TGI, ExG, NDVI, and NDWI}\DIFaddend . These indices were assessed using performance metrics, including \DIFdelbegin \DIFdel{average precision (AP)}\DIFdelend \DIFaddbegin \DIFadd{AP}\DIFaddend , recall, and F1-score. In line with previous studies \cite{su_spectral_2022}, the TGI index shows strong performance on several metrics, indicating that it can enhance detection performance for some operating points.
