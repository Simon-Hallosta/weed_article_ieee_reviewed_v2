...
Acquisition sessions used in the reported study & Although the broader image material was inspected over a wider seasonal window, all reported training, validation, and test data in this article were restricted to the selected acquisition sessions from 27~March~2023 and 12~April~2023. \\
...
To provide a threshold-aggregated summary of detection performance, we report Average Precision (AP) \cite{lin_coco_2014} together with 95\% confidence intervals (CIs) from the original bootstrap-resampling analysis \cite{efron1979bootstrap}.
...
\begin{table}[htbp]
\centering
\caption{Average Precision (AP) values for each input-channel configuration and position, along with their corresponding 95\% confidence intervals from the original 10000-resample bootstrap analysis.}
\begin{tabular}{lll}
\toprule
...
\begin{itemize}
    \item UAVs equipped with multispectral cameras and carefully selected VI-channel configurations could support more precise weed-localization workflows. Lambert et al. emphasized the importance of high-resolution imagery and advanced modeling techniques in enhancing weed management practices \cite{lambert_testing_2019}. Recent UAV-based deep-learning work also shows how detector outputs can support site-specific weed-management products such as treatment maps, although such map generation is beyond the scope of the present study \cite{mesias_ruiz_multispecies_2026}.
...
\begin{itemize}
    \item \textbf{Optimal Detection Timing:} Within the broader campaign imagery inspected for this study, the reported detector experiments used only the 27~March~2023 and 12~April~2023 sessions. These dates fall within the most useful detection period, between late March and early-to-mid April, when visual separation between black-grass and wheat was sufficient and before wheat growth caused strong occlusion of black-grass.
