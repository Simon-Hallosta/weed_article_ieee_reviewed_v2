...
Our detection model is a modified version of the YOLO architecture \cite{Jocher_Ultralytics_YOLO_2023}, adapted to accept multispectral inputs. In contrast to conventional YOLO networks that use RGB images, our baseline model uses the five aligned spectral bands captured by the UAV (Blue, Green, Red, Red Edge, and Near-Infrared (NIR); see Section~\ref{seq:VI}).
...
The investigated models are compared to a baseline \emph{5-channel} model (five spectral bands only) and to otherwise identical models where the same five bands are supplemented with one additional VI channel (NDVI, TGI, ExG, or NDWI). This is in order to analyze the impact of each vegetation index separately.
...
\begin{figure*}[t]
    ...
    \caption{Data-processing pipeline for detecting black-grass. Five aligned multispectral bands (Section~\ref{seq:VI}) are stacked together with an optional vegetation-index channel to form the model input used for both training and inference. The baseline (5-channel) model uses only the aligned bands as input.}
...
The data was collected from two winter wheat fields in the south of Sweden. In each field, we had a designated parcel of approximately 20~m $\times$ 20~m. The parcels had a recorded occurrence of black-grass. 
...
The images were captured at different stages of the black-grass growth cycle during the spring, from late February to early June.
...
The data annotation process was critical for preparing the images for training and evaluating the neural network model for weed detection. The annotations were conducted using both RGB and NDVI images as references, with human experts carefully reviewing the images to identify and mark the positions of black-grass.
...
\begin{figure*}[htbp]
    ...
    \caption{Precision-Recall (PR) curves for different models across three positional categories: Between, In row, and All. The models evaluated are the 5-channel model, TGI, ExG, NDVI, NDWI at a resolution of 1280x1280, and TGI at a resolution of 640x640. The PR curves illustrate the trade-off between precision and recall for each model, with the Average Precision (AP) indicating overall performance.}
    \label{fig:PR-curves-positions}
\end{figure*}

\subsection{Precision--recall characteristics across confidence thresholds}
As illustrated in Figure~\ref{fig:PR-curves-positions}, the precision--recall (PR) curves demonstrate that models utilizing the Triangular Greenness Index (TGI) and Excess Green Index (ExG) maintain higher precision while achieving substantial recall values across different positional categories. This robustness underscores their effectiveness in distinguishing black-grass from wheat under varied conditions. Specifically, the TGI model consistently outperforms other models in both ``between rows'' and ``in-row'' positions, indicating that it generalizes well across the two spatial contexts. Additionally, it is noteworthy that while the baseline 5-channel model achieves high recall, it suffers from lower precision at the beginning of the PR curves. This trade-off is further emphasized in the F1-score curves shown in Figure~\ref{fig:F1-curves-positions}, where the baseline model exhibits a lower overall F1-score across confidence thresholds compared to most models incorporating an additional vegetation index channel.
...
For completeness, we also report results for a TGI model trained at a reduced input resolution (640$\times$640), whereas the remaining models were trained and evaluated at 1280$\times$1280.
Table~\ref{tab:performance_metrics_by_position} presents the AP values and their 95\% confidence intervals for each model and positional category. Key observations include:
\begin{itemize}
    \item \textbf{Between Rows:} The TGI model achieves the highest AP (0.892), closely followed by the 5-channel model (0.889). The NDWI model underperforms with an AP of 0.742.
    \item \textbf{In Rows:} All models show reduced performance due to occlusion by wheat. The TGI model maintains a relatively higher performance (AP~=~0.553), closely followed by ExG (AP~=~0.547), \mbox{5-channel (AP~=~0.545)} and NDVI (AP~=~0.523).
    \item \textbf{All:} The 5-channel model achieves the highest AP of 0.805, followed by the TGI model (1280$\times$1280) with an AP of 0.800. While the baseline is marginally higher on the combined set, the TGI model performs comparatively better when the data are analyzed separately by positional category, suggesting that the overall ranking is influenced by the composition of the combined dataset.
\end{itemize}

\begin{table}[htbp]
\centering
\caption{Average Precision (AP) values for each model and position, along with their corresponding 95\% confidence intervals. Confidence intervals were computed using 10000 bootstrap resamples.}
\begin{tabular}{lll}
\toprule
\multicolumn{1}{c}{\textbf{Model}} & \multicolumn{1}{c}{\textbf{AP}} & \multicolumn{1}{c}{\textbf{95\% CI}} \\
\midrule
...
\begin{figure*}[htbp]
    ...
    \caption{F1-Confidence curves for different models across three positional categories: Between, In row, and All. The models evaluated include the 5 channel model, TGI, ExG, NDVI, NDWI at a resolution of 1280x1280, and TGI at a resolution of 640x640. The F1 score represents the harmonic mean of precision and recall, providing a balanced measure of the models' performance as a function of confidence score.}
    \label{fig:F1-curves-positions}
\end{figure*}

Figures~\ref{fig:PR-curves-positions} and \ref{fig:F1-curves-positions} depict PR behavior and F1-confidence curves, respectively. Together, these curves highlight the precision--recall trade-off when selecting confidence thresholds for practical deployment. In particular, the TGI and ExG models tend to achieve more balanced performance across thresholds, whereas the baseline 5-channel model reaches high recall at the cost of lower precision at lower confidence thresholds.

The detailed analysis reveals several key insights:
\begin{itemize}
    \item \textbf{Model Performance:} The TGI model trained on lower resolution (640x640) outperforms the NDWI model, highlighting the significance of the vegetation index relative to image resolution in this comparison.
...
\begin{itemize}
    \item The TGI index demonstrated enhanced performance, closely followed by the ExG index. Both indices outperformed the 5-channel baseline model in terms of precision while maintaining high recall values.
    \item The NDWI index underperformed compared to the other indices, negatively impacting the model’s detection capabilities.
    \item The choice of vegetation index can mitigate the limitations of lower resolution images, as seen with the TGI model at 640x640 resolution performing better than the NDWI model.
    \item The 5-channeled baseline model achieved the highest overall recall. 
\end{itemize}
...
In contrast, the NDWI index's negative values for vegetative regions might have hindered the model’s learning process, resulting in poorer performance. This hypothesis is supported by Lambert et al., who noted challenges in model transferability across different environmental conditions, which could be exacerbated by indices that do not align well with the spectral properties of the target vegetation \cite{lambert_evaluating_2018}.

The 5-channel model achieved the highest recall, indicating its effectiveness in detecting most occurrences of black-grass. However, the TGI and ExG models maintained higher precision, providing more reliable detections with fewer false positives. This trade-off is crucial depending on the application—whether the goal is to detect all weeds or to ensure precise detection with minimal errors.

\subsection{Detection Between and In Rows}
An essential aspect of our findings is the difference in detection performance between rows and within rows of wheat. Figures~\ref{fig:PR-curves-positions} and \ref{fig:F1-curves-positions}, along with Table \ref{tab:performance_metrics_by_position}, show that models are more successful in identifying black-grass between rows, where there is less occlusion and higher contrast. In-row detection is hindered by occlusion and visual similarity between wheat and black-grass, leading to lower precision and recall. For instance, the TGI model achieved an AP of 0.892 between rows compared to 0.553 within rows. This highlights the need for advanced techniques to improve in-row detection, which remains a significant challenge for UAV-based weed detection.
...
\begin{itemize}
    \item \textbf{Optimal Detection Timing:} The best detection period is between March and April when visual separation between black-grass and wheat is sufficient, and herbicide application is possible.
    \item \textbf{Image Alignment:} Accurate alignment of multispectral images is crucial. Misalignment can significantly degrade the quality of vegetation indices and the subsequent analysis.
